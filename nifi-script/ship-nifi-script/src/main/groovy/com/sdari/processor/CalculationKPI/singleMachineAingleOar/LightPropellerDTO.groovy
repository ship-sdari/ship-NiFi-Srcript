 ...
 *
 * @type: （单机单桨）
 * @kpiName: 轻浆裕度* @author Liumouren* @date 2020-09-22 10:16:00
 */
class LightPropellerDTO {
    ...
    LightPropellerDTO(final def logger, final int pid, final String pName, final int rid, GroovyObject pch) {
        log = logger
        processorId = pid
        processorName = pName
        routeId = rid
    ...
    static BigDecimal calculationKpi(Map<String, String> configMap, Map<String, BigDecimal> data, final String time, final String sid) {
        try {
            ...
            try {
                ...
                double pow = Math.pow(dfWater.divide(c, 8, BigDecimal.ROUND_HALF_UP).doubleValue(), 1.0 / 3);
// p/c的1/3次方
