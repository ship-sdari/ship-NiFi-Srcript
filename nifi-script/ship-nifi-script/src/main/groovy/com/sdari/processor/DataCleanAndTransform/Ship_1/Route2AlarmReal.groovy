...
class Route2AlarmReal {
    ...
    Route2AlarmReal(final ComponentLog logger, final int pid, final String pName, final int rid, GroovyObject pch) {
        log = logger
        processorId = pid
        processorName = pName
        routeId = rid
        ...
        for (int i = 0; i < dataList.size(); i++) {
            try {
                ...
                for (String dossKey in jsonDataFormer.keySet()) {
                    ...
                    readAlarmWays.call()
                    if (!alarmWays.contains('R') || alarmReal.containsKey(dossKey)) continue//没有实时报警方式
                    try {
