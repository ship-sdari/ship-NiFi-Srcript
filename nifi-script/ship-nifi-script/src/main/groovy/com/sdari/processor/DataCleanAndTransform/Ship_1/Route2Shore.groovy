        ...
        for (int i = 0; i < dataList.size(); i++) {
            try {
                ...
                for (String dossKey in jsonDataFormer.keySet()) {
                    try {
                        List shores = (rules?.get(sid)?.get(dossKey)?.getProperty('shore_based_distributions') as List)
                        for (shore in shores){
                            if ('A' != shore['to_shore_status'] ||
                                    null == shore['to_shore_ip'] ||
                                    null == shore['to_shore_port'] ||
                                    null == shore['to_shore_freq'] ||
                                    null == shore['column_id'] ||
