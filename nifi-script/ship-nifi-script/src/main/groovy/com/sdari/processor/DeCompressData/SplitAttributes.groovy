    ...
    static def calculation(params) {
        ...
        for (int i = 0; i < dataList.size(); i++) {
            try {//详细处理流程
                ...
                String[] alls = allStr.split('>_<')
                if (alls.size() < 5) throw "缺少文件内容或属性！" as Exception
                OutputStream out = new ByteArrayOutputStream()
                jsonAttributesFormer.put('sid',alls[1])
                jsonAttributesFormer.put('ship.collect.protocol',alls[2])
                jsonAttributesFormer.put('ship.collect.freq',alls[3])
                jsonAttributesFormer.put('decompress.type',alls[4])
